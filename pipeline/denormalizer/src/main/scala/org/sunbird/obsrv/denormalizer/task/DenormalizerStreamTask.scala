...
class DenormalizerStreamTask(config: DenormalizerConfig, kafkaConnector: FlinkKafkaConnector) {
  ...
  def process(): Unit = {
    ...
    val windowedStream: WindowedStream[mutable.Map[String, AnyRef], String, TimeWindow] =
      env.fromSource(source, WatermarkStrategy.noWatermarks[mutable.Map[String, AnyRef]](), config.denormalizationConsumer).uid(config.denormalizationConsumer)
